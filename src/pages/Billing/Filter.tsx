...
const FilterPopover = ({ filters, onApplyFilters, activeFilters }) => {
  ...
  // Initialize selected filters when component mounts or activeFilters changes
  useEffect(() => {
    if (!nonBulkFilters?.length) return;
    ...
    // If we have active filters, map them to our internal format
    if (activeFilters?.length) {
      activeFilters.forEach((activeFilter) => {
        // Find the filter group that contains this filter
        const filterGroup = nonBulkFilters.find(
          (filter) =>
            filter.portalListingId === activeFilter.PortalListingId ||
            filter.actionFilters.some((af) => af.portalListingFilterId === activeFilter.ActionFilterId)
        );

        if (filterGroup) {
          // Find the specific action filter
          const actionFilter = filterGroup.actionFilters.find((af) => af.portalListingFilterId === activeFilter.ActionFilterId);

          if (actionFilter) {
            initialSelections[filterGroup.label] = actionFilter.filterName;
          }
        }
      });
    }
    ...
    const apiFilters = Object.entries(selectedFilters)
      ...
      .map(([label, value]) => {
        ...
        return {
          ...
          // Include these IDs for persistence after refresh
          PortalListingId: filterGroup?.portalListingId,
          ActionFilterId: selectedFilter?.portalListingFilterId,
        };
      });
