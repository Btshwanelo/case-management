...
const UploadPropertyImages = () => {
  ...
  const [FacilityCreateDocumentExecuteRequest, { isLoading, isSuccess }] = useFacilityCreateDocumentExecuteRequestMutation();
  const [FacilityDocLib, { isLoading: isDeletingRecordReq }] = useFacilityDocLibMutation();
  const [FacilityDocLibUpsertRecordReq, { isLoading: isLoadingCover, isSuccess: isSuccessCover, isError: isErrorCover }] =
    useFacilityUpsertRecordReqMutation();
  const [RetrivePrevFacilityData, { data: prevData, isLoading: PrevDataIsLoading }] = useRetrivePrevFacilityDataMutation();
  const [DeleteDocumentReq, { data: dataDelete, isLoading: isLoadingDelete, isSuccess: isSuccessDelete }] = useExecuteRequest1Mutation();
  ...
  const handleDeleteFile = (documentId: string) => {
    ...
    DeleteDocumentReq({ body: payload });
  };
  ...
  const removeFile = async (index: number) => {
    try {
      if (files[index].remote) {
        console.log('Deleting remote file with recordId:', files[index].file.recordId);
        await handleDeleteFile(files[index].file.recordId);
      }
  ...
  const handleSubmit = async () => {
    ...
    await FacilityCreateDocumentExecuteRequest({ body: payload });
  };
          ...
          <div className="lg:col-span-9">
            <Card>
              ...
              <CardContent className="space-y-8">
                ...
                {PrevDataIsLoading && <Spinner />}

                {!PrevDataIsLoading && files.length > 0 && (
