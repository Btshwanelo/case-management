...
import { useFacilityRetrieveAPInspectionFeeMutation } from '@/services/facilityService';
import { useEffect } from 'react';

const PaymentInfo = () => {
  const navigate = useNavigate();
  const residentDetails = useResidence();
  const [FacilityUpsertRecordReq, { isLoading: isGeneratingInspectionInvoice }] = useFacilityUpsertRecordReqMutation();
  const [FacilityRetrieveAPInspectionFee, { data, isLoading }] = useFacilityRetrieveAPInspectionFeeMutation();

  useEffect(() => {
    FacilityRetrieveAPInspectionFee({
      body: {
        entityName: 'Facility',
