...
export const accomodationApplicationSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    TenantRegenerateLease: builder.mutation({
      query: ({ body }) => ({
    ...
    TerminateTenantReq: builder.mutation({
      query: ({ body }) => ({
        url: `/entities/ExecuteRequest?RequestName=${body.requestName}`,
        method: 'POST',
        body,
...
export const { useTenantRegenerateLeaseMutation, useTerminateTenantReqMutation } = accomodationApplicationSlice;
